...
def get_np_dates_from_arg(arg, np_obs_date_s, rounding='day', verbose=False):
    ...
    VERBOSE("Dealing with the dates using pyeq.lib.dateget_np_dates_from_arg using rounding=%s" % rounding )
    ...
    if os.path.isfile( arg ) and ( arg[-4:] == '.npy' ) :
        ...
        MESSAGE("dates read from npy file: %s" % arg )
        return np_date_s
    ...
    if os.path.isfile( arg ) and ( arg[-4:] != '.npy' ) :
        ...
        MESSAGE("dates read from text file: %s" % arg )
        return np_date_s
    ...
    if  arg =='all':
        MESSAGE("date option:all. Using all available observation dates. Rounding ignored.")
        return np.copy( np_obs_date_s )
    
    # Case all dates between two dates
    if 'all' in  arg  and '[' in  arg  and ']' in  arg :
        MESSAGE("date option: all observations within a period. Rounding ignored.")
        tmp_str= arg .replace('[','')
    ...
    # Case time step provided in days (d option)
        ...
        if 'd' in ldate[1]:
            MESSAGE("time step provided in days: %.2lf " % ldate[1])
    ...
    # Case time step provided in years (y option)
        ...
        if 'y' in ldate[1]:
            MESSAGE("time step provided in years " % ldate[1])
            # define the associated delta in seconds
    ...
    # ERROR
    ERROR("deciphering date option: %s " % arg, exit=True )
