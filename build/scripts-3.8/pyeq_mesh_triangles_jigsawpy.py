...
print('aa.shape',aa.shape)
# lon bounds
print(np.min(aa[:,0]),np.max(aa[:,0]),lon_min,lon_max)
...
opts.hfun_hmax = args.ltedge / 111.              # push HFUN limits
opts.hfun_hmin = args.ltedge / 111. /2.              # push HFUN limits
...
if np.isnan( vert[:,2]).any():
    ...
    print(np.arange(np_vert_edge.shape[0])[lindex_nan])
    print(np.arange(np_vert_edge.shape[0])[lindex_not_nan])
    print(vert[np_vert_edge[lindex_not_nan]])
...
for i in np.arange( ntria ):
    # get vertice
    [idx_v1, idx_v2, idx_v3] = mesh.tria3.shape[0][0]
    AA = np.array(coordinates.geo2xyz(vert[idx_v1][0], vert[idx_v1][1], -vert[idx_v1][2]))
    BB = np.array(coordinates.geo2xyz(vert[idx_v2][0], vert[idx_v2][1], -vert[idx_v2][2]))
    CC = np.array(coordinates.geo2xyz(vert[idx_v3][0], vert[idx_v3][1], -vert[idx_v3][2]))
    ...
    geometry[i,7] = strike
    geometry[i,8] = dip
    geometry[i,9:10] = [l, phi]
    geometry[i,11] = (vert[idx_v1]+vert[idx_v2]+vert[idx_v3])
    geometry[12:14] = vert[idx_v1]
    geometry[15:17] = vert[idx_v2]
    geometry[18:20] = vert[idx_v3]
    geometry[i,21] = area
