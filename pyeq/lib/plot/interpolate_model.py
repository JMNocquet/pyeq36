def interpolate_model( model , geometry ):
    ...
    pyeq.lib.plot.model2shp_gmt( geometry, 'tde', lslip_dat, out_dir_shp=model.odir+'/shapefile/i_slip_cumulative', out_dir_gmt=model.odir+'/gmt/i_slip_cumulative' , verbose=model.verbose )
    
    lslip_dat=glob.glob(model.odir+"/slip/i_rate/*_slip_rate.dat")
    pyeq.lib.plot.model2shp_gmt( geometry, 'tde', lslip_dat, out_dir_shp=model.odir+'/shapefile/i_slip_rate', out_dir_gmt=model.odir+'/gmt/i_slip_rate' , verbose=model.verbose )
    
    ###########################################################################
    # PLOT
    ###########################################################################

    pyeq.lib.plot.plot_model( model , interpolation=True )
