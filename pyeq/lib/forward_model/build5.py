def build5( model ):
    ...
    print("-- BUILDING THE NORMAL SYSTEM USING PYEQ.LIB.BUILD5 --")
    ...
    # GAMMA_MATRIX
    GAMMA_MATRIX = pyeq.lib.forward_model.make_GAMMA_MATRIX(model.t_obs, model.np_obs_date_s, model.np_model_date_s)

    # DICTIONNARY OF OBS INDEX k AT EACH MODEL STEP i
    H_i_k = pyeq.lib.forward_model.make_auxiliary_index( model )
    ...
    for model_time_step in np.flip( np.arange( model.nstep ) ):
        ...
        for k in sorted( lk ):
            ...
            #print( GAMMA_MATRIX )
            # Get G
            G = pyeq.lib.forward_model.make_G_at_k( model , model_time_step , CDK , GAMMA_MATRIX )
