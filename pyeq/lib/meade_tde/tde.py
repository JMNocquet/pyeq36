...
from . import disp_functions as df

try:
    import strain_functions_cy as sf
except ImportError:
    from . import strain_functions as sf


'''

from https://github.com/cossatot/tri_dislocations_python
...
def calc_tri_strains(sx=None, sy=None, sz=None, x=None, y=None, 
    ...
    for i_tri in [0, 1, 2]:
        strike, _dip, beta, lss, lts, lds = get_edge_params(i_tri, xx, yy, zz, 
                                                          slip_vec)
...
#    seg_map_length = np.sqrt(x_dist**2 + y_dist**2)
    rx, _ry = rotate_xy_vec(x_dist, y_dist, -strike)
...
def rotate_xy_vec(x, y, alpha):
    '''Rotates a vector by an angle alpha'''
